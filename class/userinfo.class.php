...
//        $names = $this->query("Select * from ecm_member where user_name='{$username}' and password='{$password}'");
        $namen = $this->get_row("Select * from ecm_member where user_name='{$username}' and password='{$password}'");
        if ($namen <> 0) {
            $cookie = array(
                "username" => $name,
                "password" => $password,
...
//        $names = $this->query("Select * from ecm_member where user_name='{$username}' and password='{$password}'");
    ...
    function user_info($name){
        $user_info = $this->get_row("Select * from user where user_name='{$name}'");
        return $user_info;
    }
}